...
const COPY: Record<Lang, any> = {
  en: {
    ...
    ai: "AI",
    desc2: " analyzes what suits you and selects the piece made for you — matched to your ",
  ...
  es: {
    ...
    ai: "IA",
    desc2: " analiza lo que te favorece y te recomienda la pieza hecha para ti — en función de tu ",
  ...
  >
    <svg viewBox="0 0 64 64" className="absolute inset-0 h-full w-full">
      ...
      <g clipPath="url(#drop)">
        ...
        <motion.rect x="-96" y="0" width="96" height="64" fill="url(#gloss)" initial={{ x: -96 }} animate={{ x: 128 }} transition={{ duration: 3.6, repeat: Infinity, ease: "easeInOut", repeatDelay: 1.2 }} />
      </g>
      <path d="M32 6 C38 14, 46 22, 50 31 C54 40, 48 54, 32 58 C16 54, 10 40, 14 31 C18 22, 26 14, 32 6 Z" fill="none" stroke="#ffffff" strokeOpacity=".7" />
    </svg>
  </motion.div>
);
...
const TypeTrait = ({ lang }: { lang: Lang }) => {
  ...
  const longest = useMemo(() => traits.reduce((m, s) => Math.max(m, s.length), 0), [traits]);
...
const Step = ({ no, icon, title, desc }: { no: string; icon: React.ReactNode; title: string; desc: string }) => (
  <div className="relative flex items-start gap-3 p-4 rounded-2xl ring-1 ring-rose-200/60 bg-white/70 backdrop-blur supports-[backdrop-filter]:bg-white/60">
    <div className="relative shrink-0">
      ...
      <span className="absolute -top-1 -left-1 h-5 w-5 rounded-full bg-white text-rose-600 ring-1 ring-rose-300 grid place-items-center text-[10px] font-semibold">{no}</span>
    </div>
